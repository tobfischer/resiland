...
def two_FHN_NoN_f():
    ...
    yield rl.coupling_to_potential(now, amp, mu, var, Z ,can, gamma, X, N) ### dZdz



def generate_time_series(amp, mu, var, initial_z, times, cantor_position, k_int2_list, k_b_list):
    ...
    cantor_position :                       2d array of the positions of the start- and endpoint of the Cantor sets
    ...
    number_cantor,_ = shape(cantor_position)
    cantor_pos = [rl.create_cantor(4, cantor_position[i][0], cantor_position[i][1]) for i in range(number_cantor)]

    for time in times:
        get_control, amp, mu, var = rl.return_set_pa(k_int2, k_b, gamma, can, amp, mu, var)
        sigma = nsqrt(var)
        ...
        for i in range(number_cantor):
            if cantor_pos[i][0] < z[-1] < cantor_pos[i][-1]:
                can =  rl.cantor_slope(z[-1], cantor_pos[i], steepness, amp, mu, sigma)
        ...
        if(len(events)!=0 and time == events[-1][0]):
            gamma = rl.sign_change(gamma, z, -4 , 4, random = False)
...
if __name__== '__main__':
    ...
    now = 4 ### number of wells in the potential landscape
    ...
    k_int2_list = [0.116, 0.116, 0.125, 0.125]              ### global coupling strength for network 2 (different for each well)
    k_b_list    = [1.045e-5, 1.425e-5, 1.045e-5, 1.5e-5]    ### global coupling strength between network 1 and 2 (different for each well)


    X, Y, Z, Sym, helpers = rl.create_dynamic_variables(N)  
    
    k_int2, k_b, gamma, can = rl.symbol_dynamics()
    amp, mu, var = rl.symbol_potential(now)

    
    cantor_position = [
        [0,0],
    ] ### start- and endposition of the each Cantor set

    steepness  = 0.3 ### steepness parameter of the potential landscape inside the fragmented barrier

    control_para = rl.make_control_pars(k_int2, k_b, gamma, can, now)

    I = rl.prepare_jitcode(two_FHN_NoN_f, helpers, N, control_para)

    amp = [5.5, 5.5, 5.5, 5.5]                                  ### control parameters of the amplitude of the potential landscape
    mu  = [-6, -2, 2, 6]                                        ### control parameters of the mean of the potential landscape
    var = [2, 2, 2, 2]                                          ### control parameters of the variance of the potential landscape
   
    x_1, y_1, x_2, y_2, z = generate_time_series(amp, mu, var, -1, range(1000, 101000), cantor_position, k_int2_list, k_b_list)
