...
def generate_time_series(amp, mu, var, initial_z, times, k_int2_list, k_b_list, fragment_barrier_position = [[0,0]]):
    ...
    shift_start, shift_stop = 10000, 25000
    ...
    for time in times:
        ...
        if shift_start < time < shift_stop:
            mu[3] = 22.5 - 4*(time-shift_start)/(shift_stop-shift_start)
        ...
        if(len(events)!=0 and time == events[-1][0]):
            gamma = rl.sign_change(gamma, z, -16 , 16, random = False)
...
if __name__== '__main__':
    ...
    amp = [36.5, 36.5, 36.5, 36.5]                          ### control parameters of the amplitude of the potential landscape
    mu  = [-18.3, -6.2, 6.2, 22.5]                          ### cantrol parameters of the mean of the potential landscape
    var = [12, 12, 12, 12]                                  ### control parameters of the variance of the potential landscape
   
    x_1, y_1, x_2, y_2, z = generate_time_series(amp, mu, var, -1, range(1000, 101000), k_int2_list, k_b_list, fragment_barrier_position)
